...
""";
  ...
  // 为滚动弹幕挑选轨道:优先用右侧已完全空出的一条(startSec >= 空出时间),
  // 若全部占用则选最早空出的一条,尽量减少视觉重叠。
  static int _pickRollChannel(List<double> channelFreeAt, double startSec) {
    int best = 0;
    double earliest = double.infinity;
    for (int c = 0; c < channelFreeAt.length; c++) {
      final freeAt = channelFreeAt[c];
      if (startSec >= freeAt) return c;
      if (freeAt < earliest) {
        earliest = freeAt;
        best = c;
      }
  ...
  static String convert(String xmlContent, {DanmakuOptions? options}) {
    ...
    try {
      ...
      dNodes.sort((a, b) {
        ...
        return (double.tryParse(pa[0]) ?? 0).compareTo(
          double.tryParse(pb[0]) ?? 0,
        );
      });

      // Layout parameters
      final double lineSpacing = opt.fontSize * 1.3;
      final int maxChannels = ((opt.resY * opt.area) / lineSpacing)
          .floor()
          .clamp(1, 30);
      // 每条滚动轨道“右侧空出”的时间点,用于防重叠布局
      final List<double> channelFreeAt = List.filled(
        maxChannels,
      ...
      for (var node in dNodes) {
        ...
        final assColor =
            "&H00${blue.toRadixString(16).padLeft(2, '0').toUpperCase()}${green.toRadixString(16).padLeft(2, '0').toUpperCase()}${red.toRadixString(16).padLeft(2, '0').toUpperCase()}";
        ...
        if (mode <= 3) {
          ...
          int channel;
          if (opt.noOverlap) {
            // 防重叠:弹幕匀速左移,其尾部完全进入屏幕所需时间 = 文字宽度 / 速度
            final velocity = (xStart - xEnd) / duration; // px/s
            final enterTime = velocity > 0 ? textWidth / velocity : 0.0;
            channel = _pickRollChannel(channelFreeAt, startSec);
            channelFreeAt[channel] = startSec + enterTime;
          } else {
            channel = rollChannelCounter % maxChannels;
            rollChannelCounter++;
          }

          final yPos = (opt.resY * 0.05) + (channel * lineSpacing);
          events.add(
            "Dialogue: 0,$tStart,$tEnd,Roll,,0,0,0,,$colorTag{\\move($xStart,$yPos,$xEnd,$yPos)}$text",
          );
        } else if (mode == 5) {
      ...
      debugPrint("Error converting danmaku: $e");
